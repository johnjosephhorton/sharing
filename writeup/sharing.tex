...
We also compare how bringing-to-market costs to affects the analysis. 
...
\cite{belk2014you}
\cite{bolton2013engineering}
\cite{cabral2010dynamics}
\cite{cannon2014uber}
\cite{chang2015growing}
\cite{cullen2014outsourcing}
\cite{dellarocas2003digitization}
\cite{dinerstein2014consumer}
\cite{edelman2015efficiencies}
\cite{einav2015peer}
\cite{fradkin2012online}
\cite{fradkin2013search}
\cite{fradkin2015}
\cite{fradkin2015bias}
\cite{gata2015sharing}
\cite{hagiu2014strategic}
\cite{hampshire2011peer}
\cite{heinrichs2013sharing}
\cite{hui2014lemon}
\cite{koopman2014sharing}
\cite{neeser2015does}
\cite{resnick2000reputation}
\cite{resnick2002trust}
...
Our third and final question is how the usage patterns for different goods is likely to affect bringing-to-market costs.
...
\end{align}
The new market-clearing rental rate is simply the rental rate when there are no bringing-to-market costs (from Equation~\ref{eq:strr}) plus the per-unit transaction cost scaled by the the size of supply side of the market, or  
...
The highest possibility bringing-to-market cost that will still support a P2P rental market is 
\begin{align} 
...
Returning to the case where we assume bringing-to-market costs are zero, we now consider what happens in the long-run, when can revise their ownership decisions. 
...
when we assumed bringing-to-market costs, ownership was tiled towards those placing a high value on their own consumption.
...
In the model, the purchase price, the valuation of non-owners and non-owners, the size of the pool of owners and non-owners, and the bringing-to-market costs of a good all affected whether a P2P rental market was possible.
...
In the language of our model, these are goods with high $p$, broad-based appeal (meaning a large pool of non-owners with a non-zero $\alpha$) and a sufficiently low bringing-to-market cost, or low $\gamma$.
