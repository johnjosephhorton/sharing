...
% TODO: Better title?
% TODO: Cite to that Slee guy; Dean Baker and the platform cooperative guy
...
Our model is built from the notion that goods can usefully be thought of as having an intensive margin of usage, which in turn drives the extensive margin decision (i.e., ownership), which is conditioned upon price. 
...
We first consider what happens when the possibility of P2P rental emerges.
We assume that there are no bringing-to-market costs (such as labor and transaction costs) and determine the equilibrium rental rate and quantity transacted.
This allows us to characterize the surplus ``unlocked'' by the short-run P2P rental market.

Next we introduce bringing-to-market costs and see how this changes the short-run equilibrium. 
We show that with sufficiently high bringing-to-market costs a P2P rental market cannot exist, which is consistent with the argument that technological advances are the proximate cause for the emergence of P2P rental markets in recent years.
...
Consider a marketplace with two consumer types: $\alpha_H$ and $\alpha_L$ with $\alpha_H > \alpha_L$, implying that those with a high valuation, $H$-types, would use the good more than those with a low-valuation, the $L$-types.
...
The upper-right triangle labeled ``Both buy'' is the region where both consumer-types buy and the lower-left triangle where neither buys.  
