...
\begin{align}
\theta_i u(x^*) - c x^* - p \ge 0.  
\end{algin} 
Intuitively, those consumers that will use the product more conditional upon purchase are more likely to purchase. 
If $\underline{\theta}(p)$ is the lowest $\theta$ consumer that still purchases the product, then the market demand curve will be 
\begin{align}
D(p) = n \left(1 - F(\underline{\theta(p)})\right)
\end{align} 
where $F$ is the cdf of consumer types. 

\subsection{Two consumer types with and without rental markets} 
Consider the case where there are just two consumer types: $\theta_H$ and $\theta_L$, with $\theta_H > \theta_L > 0$. If $p$ is low-enough, then both types purchase. 
If $p$ is high-enough, neither type purchases. 
For a range of prices betweent these extremes, the high-types purchase the good and the low-types go without. 

Now we consider what happens when individual purchasers can rent their excess capacity---$1-x$---to non-purchasers via peer-to-peer rental markets. 
To begin, no one can revise their original purchase decision, but they can revise their personal usage.  
The purchasers rent to the non-purchasers at a rental rate $r$. 

Let us assume that $\tau$ of the consumers are high-types and $1-\tau$ are low-types. 

\paragraph{Rentals in the high-types are the only purchasers market} 

Now consider the possibility that purchasing consumers can rent out whatever capacity they do not use at a rental rate $r$. 
For example, a non-purchaser renting for $x$ units of time will pay $rx$. 
For a purchaser, the full cost of usage is now $c + r$, since usage has both the direct cost and the opportunity cost of foregone rental. 
The the non-purcahser, the cost of usage is the same, since they have to pay $r + c$ as well.  
There is a fixed cost $\gamma$ of renting paid by purchasers of the good.  
For this reason, at an equilibrium rental rate 
\begin{align}
\theta_H u'(x_H^*(c,r)) = \theta_L u'(x_L^*(c,r)) = c + r. 
\end{align} 
Furthermore, for the market to clear, 
\begin{align}
1 - x_H^*(c, r)  = \left(\frac{1-\tau}{\tau}\right)  x_L^*(c, r). 
\end{align} 
Also, $\left(1-x_H^*(x,r)\right)r > \gamma$. 

\begin{prop}
In the equilibrium of the peer-to-peer rental market, the high-types still consumer more of the good than the low-types.  
\end{prop} 
\begin{proof} 
Because $\theta_H > \theta_L$, $u'(x^*_H) < u'(x^*_L)$ and by the concavity of $u$, $x^*_H > x^*_L$. 
\end{proof} 

\subsection{Comparative statics of the rental rate} 
\begin{prop}
If the marginal cost of usage goes up ($c$ increases), rental rates fall, as will ownership.   
\end{prop} 
\begin{proof}
For both kinds of consumers, an increase in $c$ reduces equilibrium consumption by a simple sign argument and the assumption that $u''(\cdot) < 0$:  
\begin{equation}
\theta u''(x^*)\frac{\partial x^*}{\partial c} = 1.  
\end{equation} 
A decrease in $x_H^*$ raises $1-x_H^*$, meaning there is more excess capacity on the market available for rent, but on the demand side, $x_L^*$ falling means less demand. 
The result is a lower-rental rate, which causes more renting as purchasers can revise their purchasing decisions.
\end{proof} 

\subsection{Revising the ownership decision} 
Utility for high-type purchasers renting out their excess capacity is  
\begin{align}
\theta_H u(x^*_H) - cx^*_H + (1 - x^*_H)r  - p - \gamma 
\end{align} 
whereas if they had counter-factually rented, their utility would have been 
\begin{align}
\theta_H u(x^*_H) - c x^*_H  - r x^*_H  
\end{align} 
A consumer would have preferred purchasing to renting if 
\begin{align}
 \theta_H u(x^*_H) - cx^*_H + r(1 - x^*_H)  - p  >  \theta_H u(x^*_H) - c x^*_H  - r x^*_H 
\end{align} 
which simplifies to $r > p + \gamma$. 
