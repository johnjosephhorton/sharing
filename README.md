...
% The ``sweet spot'' for lending appears to be goods with ``middle'' rates of ownership:  
...
% Figure~\ref{fig:scatter_lending} shows the scatter plot of fraction of respondents lending a good versus the fraction owning a good. 
...
% \subsection{Income effects are driver of differences in ownership} 

% Let $w$ be a user's wealthy. 
% Utility if purchased: $\alpha^2 - U(w - p)$, otherwise, $u(w)$ 

% Let $\underline{w}$ be the minimum wealth of someone who would buy. 

% Let $w_L$ and $w_H$.

% Consume 
% \begin{align}
% \argmax 2\alpha x - x^2 - rx + U(w - rx) 
% \end{align} 
 
% $2\alpha - 2x - r + U'(w - rx)r = 0$

% For all $x$, $U'_L \approx U'(w_L - r x)$ and  $U'_H \approx U'(w_H - r x)$. 

% \begin{align}
% x = \frac{U'(w-rx)r - r + 2\alpha}{2} = \alpha - r \left( \frac{1 + U'(w-rx)}{2} \right)
% \end{align} 

% \begin{align}
% x_H = \alpha - r \frac{1 + U'_H}{2} \quad x_L = \alpha - r \frac{1 + U'_L}{2}
% \end{align} 
